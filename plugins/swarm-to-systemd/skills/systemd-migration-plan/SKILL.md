...
- `inventory.json` from `swarm-capture` (required).
- `rendered/MIGRATION-NOTES.md` and `rendered/expected.json` from `swarm-to-quadlet` (strongly recommended; plan without them only when the user asks for an early estimate).
