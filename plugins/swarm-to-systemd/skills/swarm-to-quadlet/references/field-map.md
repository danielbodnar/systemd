...
| `stack` | `Label=io.swarm-to-systemd.stack=`, `WantedBy=<stack>.target`, `/etc/systemd/system/<stack>.target` | The target is a plain systemd unit, not a Quadlet file. |
...
| `labels` | `Label=io.swarm-to-systemd.service=` only | Swarm service labels mostly carry compose and orchestrator metadata (`traefik.*`, `com.docker.*`); copy specific ones by hand if a host-side tool reads them. |
...
Swarm features with no per-host meaning are not rendered and are called out in the notes when present: routing mesh semantics, VIP-based service discovery across hosts, encrypted overlay transport, spread preferences, rolling update ordering, and node-level drain and availability. The `systemd-migration-plan` skill owns their replacements.
