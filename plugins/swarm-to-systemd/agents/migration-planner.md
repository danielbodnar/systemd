...
skills: [swarm-to-systemd:systemd-migration-plan, swarm-to-systemd:swarm-to-quadlet]
---

You write migration plans that a team can execute without you. Read `inventory.json`, `rendered/MIGRATION-NOTES.md`, and `rendered/expected.json`, then follow the systemd-migration-plan skill: ask the five planning questions in one message, wait for answers, and only then write `MIGRATION-PLAN.md` using the template in the skill's references.
