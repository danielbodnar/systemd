...
#
# Install the swarm-agent worker on a production host. Run it from a checkout
# of the systemd-dev-plugins tree; it lives under
# plugins/systemd-migration-harness/harness/systemd/ and copies the sibling
# plugins next to the harness.
#
...
# The skills and the inventory contract of every plugin in the marketplace,
# laid out as in the repository, so an operator-run capture or render on this
# host uses the same scripts the agents were applied with.
plugins_dir="$(cd "$source_dir/../.." && pwd)"
rm -rf "$prefix/plugins"
for plugin in "$plugins_dir"/*/; do
    name="$(basename "$plugin")"
    [ -d "$plugin/skills" ] || continue
    install -d -m 0755 "$prefix/plugins/$name"
    cp -a "$plugin/skills" "$prefix/plugins/$name/skills"
    [ -d "$plugin/contract" ] && cp -a "$plugin/contract" "$prefix/plugins/$name/contract"
done
