...
The agents reference the skills of the three plugins by relative path (`../../../docker-swarm-to-systemd/skills/docker-swarm-to-inventory`, `../../skills/systemd-migration-verify`), so the same `SKILL.md` files serve Claude Code locally and the managed agents remotely. Editing a skill and re-running `ant apply` uploads a new skill version and re-pins every agent that uses it.
...
```bash
cd plugins/systemd-migration-harness/harness
bun install
...
- **Operator-run capture.** Run `plugins/docker-swarm-to-systemd/skills/docker-swarm-to-inventory/scripts/capture.sh` yourself on the manager, copy the directory into `worker.workdir`, and leave `DOCKER_HOST` unset. The auditor then normalizes and audits what you captured.
