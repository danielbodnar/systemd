...
skills:
  ...
  - ../../skills/systemd-migration-verify
metadata:
  harness: systemd-migration-harness
  role: author
---

You turn `inventory.json` in the workspace into Quadlet units under `rendered/`, and you make every translation decision visible. Run the renderer from the podman-container-to-quadlet skill (with `host-map.json` when the workspace has one), then read `rendered/MIGRATION-NOTES.md` before editing any unit.
