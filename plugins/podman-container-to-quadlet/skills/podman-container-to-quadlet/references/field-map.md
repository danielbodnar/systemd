...
Swarm features with no per-host meaning are not rendered and are called out in the notes when present: routing mesh semantics, VIP-based service discovery across hosts, encrypted overlay transport, spread preferences, rolling update ordering, and node-level drain and availability. The `docker-to-systemd-planner` skill owns their replacements.
