...
export function render(inv: Inventory, opts: RenderOptions): RenderResult {
  ...
  for (const svc of [...inv.services].sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    for (const [hostname, count] of placement) {
      ...
      for (let i = 1; i <= count; i++) {
        ...
        const u = new Unit([
          `Rendered by podman-container-to-quadlet from swarm service ${svc.name} (stack ${svc.stack ?? "none"}, mode ${svc.mode})`,
  ...
  for (const plan of Object.values(hosts)) {
    for (const name of plan.networks) {
      ...
      if (net.driver === "overlay" && net.used_by.length > 1) {
        notes.push(`network ${net.name}: overlay spanned hosts; on systemd each host gets a local bridge with the same subnet, so cross-host members need routed or WireGuard connectivity (see docker-to-systemd-planner/references/networking.md)`);
      }
    }
  ...
  if (inv.services.some((s) => s.endpoint_mode === "vip" && s.networks.length > 0)) {
    notes.push("service discovery: Swarm VIPs are replaced by Podman network DNS; an alias resolves only to containers on the same host's bridge, so cross-host callers need DNS or a load balancer (see docker-to-systemd-planner/references/networking.md)");
  }
