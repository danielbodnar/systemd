...
import { nsToDuration } from "../../../contract/types.ts";
...
export function normalize(dir: string, opts: { keepEnvValues?: boolean } = {}): Inventory {
  ...
  for (const n of nets) if (n.encrypted) warnings.push(`network ${n.name}: encrypted overlay; cross-host transport must be replaced (see docker-to-systemd-planner/references/networking.md)`);
  const inv: Inventory = {
    version: "1",
