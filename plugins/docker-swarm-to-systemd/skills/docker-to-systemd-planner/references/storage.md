...
For each volume record the file count and byte size on both sides and a content hash of a sample; `systemd-migration-verify` can run those checks when they are written as commands in the plan.
