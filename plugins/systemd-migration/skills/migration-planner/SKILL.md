...
- `inventory.json` from `discover-docker-swarm` (required).
- `rendered/MIGRATION-NOTES.md` and `rendered/expected.json` from `podman-quadlet` (strongly recommended; plan without them only when the user asks for an early estimate).
