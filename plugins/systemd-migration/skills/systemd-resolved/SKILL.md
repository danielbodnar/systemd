...
description: The name resolution component. Replaces the orchestrator's embedded DNS (service names, network aliases, dnsrr endpoints) with one of three mechanisms chosen in plan.yaml: rendered /etc/hosts entries that point at a host or at a machine's static lease on its zone bridge, systemd-resolved with DNS-SD announcements per host (.dnssd files, mDNS and LLMNR on the zone), or records in the site's DNS. A service that took a virtual address from a multipath publish decision is named at that address instead of at every host that runs it. Use this whenever the user asks how services find each other after the migration, how a machine on a zone bridge is named, about resolved.conf, resolvectl, DNS-SD, mDNS, LLMNR, LocalLeaseDomain, service aliases, or /etc/hosts.
---
...
A service whose published port chose `multipath` has an address of its own (`networkd.vip.<service>`), and the same address is what every host in its ingress scope carries or routes. The `hosts` fragment therefore lists such a service once at that address instead of once per host, and the other two mechanisms say so in their notes: the `dnssd` announcement names the host it is made on, so a client that finds the service that way reaches one instance directly and bypasses the multipath route, and the `site-dns` list names the virtual address as the single record to publish.
