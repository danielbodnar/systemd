...
**Environment redaction.** Redaction happens twice. `capture.sh` redacts an environment entry when its key matches password, pass, pwd, secret, token, api key, private key, credential, or auth, or when its value carries URI userinfo with a password (`scheme://user:pass@`) or a `password=`-style pair; it applies the same value test plus `--password=x`, `-px`, and bare `--password VALUE` forms to `Command` and `Args`, and drops `PreviousSpec`, all before the raw file is written (`--keep-env` retains everything and is a deliberate choice). Long opaque strings without such markers are not detected, so review `services.json` before sharing a capture. The normalizer applies the same key and value tests again; matches become `<redacted>` and are listed in `redacted_env`. `--keep-env-values` on the normalizer only keeps what the capture kept.
