...
3. Hand `inventory.json` to `podman-quadlet` (rendering) and `migration-planner` (planning). Keep the raw directory next to it; reviewers will want to check a rendered unit against the original inspect output.

```bash
# on a manager node
bash "${CLAUDE_PLUGIN_ROOT}/skills/discover-docker-swarm/scripts/capture.sh" -o /tmp/migrate-discover --compose-dir /srv/stacks

# on the operator machine
bun "${CLAUDE_PLUGIN_ROOT}/skills/discover-docker-swarm/scripts/normalize.ts" ./migrate-discover -o ./inventory.json
```

If the user works from a project directory, write both outputs under the plugin's configured inventory directory (default `.systemd-migration/`) so later commands find them without being told where to look.

## What the inventory contains

The schema lives in `contract/inventory-schema.json` at the plugin root (JSON Schema 2020-12) and is the contract between every plugin in the marketplace; `contract/README.md` explains how it is vendored. Read it when you need a field's exact name or type; the summary is:
...
- `../../contract/inventory-schema.json`, `types.ts`, `schema.ts`: the inventory contract, its types, and the validator, shared with every other plugin.
...
- `references/example-inventory.json`: a small two-node, two-stack inventory used by the harness tests and useful as a template when the user has no cluster access yet.
