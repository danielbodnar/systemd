...
export function normalize(dir: string, opts: { keepEnvValues?: boolean } = {}): Inventory {
  ...
  for (const n of nets) if (n.encrypted) warnings.push(`network ${n.name}: encrypted overlay; cross-host transport must be replaced (see migration-planner/references/networking.md)`);
  const inv: Inventory = {
    version: "1",
