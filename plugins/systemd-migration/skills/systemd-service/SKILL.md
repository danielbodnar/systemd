...
```bash
bun "${CLAUDE_PLUGIN_ROOT}/skills/systemd-service/scripts/render.ts" inventory.json -o rendered-native \
...
5. Pull the images on each host with the `systemd-machined` skill (`pull-images.sh rendered-native/images.json`), fill `/etc/swarm-migration/secrets/` and run `secrets/import-credentials.sh`, then `install.sh` from the host's directory. Verify with the `systemd-verify` skill before starting anything.

Flags: `--host-map FILE` overrides placement (`{"service": ["host-a"]}`); `--scale-out` renders numbered instances when a service wants more replicas than eligible hosts; `--root-image` writes `RootImage=NAME.raw` for hosts without mount stacks (build the images with `systemd-machined`); `--image-dir` and `--state-dir` change where images and volumes live on the hosts.
