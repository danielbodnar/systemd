...
export function render(inv: Inventory, opts: RenderOptions): RenderResult {
  ...
  if (inv.services.some((s) => s.endpoint_mode === "vip" && s.networks.length > 0)) {
    notes.push("service discovery: Swarm VIPs are replaced by Podman network DNS; an alias resolves only to containers on the same host's bridge, so cross-host callers need DNS or a load balancer (see migration-planner/references/networking.md)");
  }
