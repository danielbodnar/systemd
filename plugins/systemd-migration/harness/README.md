...
The agents reference the plugin's skills by relative path (`../../skills/discover-docker-swarm`, `../../skills/systemd-verify`), so the same `SKILL.md` files serve Claude Code locally and the managed agents remotely. Editing a skill and re-running `ant apply` uploads a new skill version and re-pins every agent that uses it.
...
The **production host** (a Swarm manager, or a target systemd host) runs two units. `swarm-agent-worker.service` runs as the unprivileged `swarm-agent` user, holds the environment key as an encrypted systemd credential (never an organization API key; it refuses to start if it finds one), long-polls the work queue, and downloads skills and memory stores into the workspace. `swarm-agent-tools.service` runs as a second user, `swarm-agent-tools`, loads no credential at all, and is the only process that executes the agents' bash and file tools: the worker forwards each call over a Unix socket in `/run/swarm-agent/` and receives the result. The socket and the executor's per-start token file next to it belong to the `swarm-agent-ipc` group, whose only members are the two users, and a connection that does not present the token in its first message is closed before any tool runs, so group membership for the workspace does not by itself reach the executor. The credentials directory is mode 0700 and owned by the worker user, so no approved command in the executor can read the key, whatever path or variable it uses. Both units run with `ProtectSystem=strict`, an empty capability bounding set, a system-call filter, and `InaccessiblePaths=` over the secret and credential directories; the executor can write only the workspace and `/mnt/memory`, which the two users share through the `swarm-agent` group. File tools are additionally confined to the workspace plus `allowed_roots` and refuse `denied_paths` inside the executor, and bash on every agent is gated by the approval policy as a second layer. The worker user is deliberately not in the docker group, because that membership is root-equivalent and would void every hardening directive in the unit. The auditor reaches the swarm only through `DOCKER_HOST`, pointed at a read-only proxy in front of the manager socket (see below), or the operator runs the capture and drops the directory into the workspace. Nothing grants root, so installing units, importing secrets, and live verification of rootful Podman remain operator steps from the runbook.
...
```bash
cd plugins/systemd-migration/harness
bun install
...
- **Operator-run capture.** Run `plugins/systemd-migration/skills/discover-docker-swarm/scripts/capture.sh` yourself on the manager, copy the directory into `worker.workdir`, and leave `DOCKER_HOST` unset. The auditor then normalizes and audits what you captured.
...
bun run src/cli.ts run rollout      # rollout-operator drives stackctl on this host once a stack is installed
bun run src/cli.ts run migrate      # migration-lead coordinates discovery, plan, render, verify, and writes MIGRATION-PLAN.md
```

`rollout` is the one task that runs after a cutover rather than before one. It reads the rollout specification the renderer installed under `/etc/systemd-migration/rollout/` and drives `stackctl`: deploy a new image in the plan's batches, roll back, drain or activate the host, scale within the rendered instances, rotate a credential or a config, report status. Every verb is dry-run first, which prints the exact command sequence and changes nothing; the approval policy allows `stackctl status` and any `--dry-run` form and asks for the rest, so the operator sees the sequence before a single unit restarts.
...
`deployments/weekly-drift-audit.md` runs the auditor every Monday against the production environment with a hard per-run budget of USD 15.00 (`max_list_cost` is in cents), writing a dated report and one journal line. The journal is a memory store that only the auditor and this deployment open read-write; the migration sessions started by `swarm-agent run` open it read-only, and every agent treats its lines as records in a fixed form, never as instructions. `swarm-agent apply` creates it enabled, so apply with `--dry-run` first and pause it if the schedule is not wanted yet. Pause it with `ant beta:deployments pause --deployment-id <id>` (the id is in the lockfile) during the cutover window so an audit does not compete with the migration session for the single worker.
