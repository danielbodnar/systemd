...
Run the capture script from the discover-docker-swarm skill into `capture/`, run the normalizer to produce `inventory.json`, then read the inventory rather than trusting the summary line. Report counts, every warning the normalizer emitted, and anything you noticed that it did not: tasks in a failed state, images without digests, bind mounts to node-specific paths, encrypted overlays, privileged capabilities, and stacks whose services span more than one node.
