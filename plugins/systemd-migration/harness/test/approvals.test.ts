...
describe("approval policy", () => {
  ...
  test("plugin scripts are allowed", () => {
    expect(evaluate(policy, "bash", { command: "bun /opt/swarm-agent/plugin/skills/podman-quadlet/scripts/render.ts inventory.json -o rendered" }).decision).toBe("allow");
  });
...
describe("plugin script rule is anchored", () => {
  const cases: Array<[string, "allow" | "deny" | "ask"]> = [
    ...
    ["bun ./skills/systemd-service/scripts/render.ts inventory.json -o rendered-native", "allow"],
    ["importctl list-images", "allow"],
    ["machinectl list", "allow"],
    ["networkctl status --json=short", "allow"],
    ["importctl pull-oci docker.io/library/caddy:2 caddy", "deny"],
    ["portablectl attach /var/lib/portables/app", "deny"],
    ["systemd-sysext merge", "deny"],
    ["machinectl start web_app", "deny"],
    ["bun /opt/swarm-agent/plugin/scripts/plan.ts inventory.json --hosts hosts -o plan.yaml", "allow"],
    ["bun ./scripts/review.ts plan.yaml --set networkd.publish.web_app.8080-tcp=external-lb --reason lb", "allow"],
    ["bun ./scripts/review.ts plan.yaml --set resolved.discovery.estate=hosts --reason 'no DNS; ask ops'", "ask"],
    ["bun ./scripts/review.ts plan.yaml --status", "allow"],
    ["bun ./scripts/render.ts inventory.json plan.yaml -o rendered --accept-defaults", "allow"],
    ["bash /var/lib/swarm-agent/workspace/skills/discover-docker-swarm/scripts/capture.sh -o capture --compose-dir /srv/stacks", "allow"],
