...
# plugins/systemd-migration/harness/systemd/ and copies the plugin's skills
# and contract next to the harness.
#
...
# The plugin's skills, scripts, and contract, laid out as in the repository,
# so an operator-run discovery or render on this host uses the same scripts
# the agents were applied with.
plugin_dir="$(cd "$source_dir/.." && pwd)"
rm -rf "$prefix/plugin"
install -d -m 0755 "$prefix/plugin"
for part in skills scripts contract; do
    [ -d "$plugin_dir/$part" ] && cp -a "$plugin_dir/$part" "$prefix/plugin/$part"
done
