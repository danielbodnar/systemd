...
`rendered/native/` is the estate rendered with every service as a plain service; `rendered/machine/` is the same estate rendered from `plan-machine.yaml`, a committed, fully reviewed plan that runs `web_app` as a machine (`systemd-nspawn@web_app.service` with a `.nspawn` file), so the nspawn subtest can start it. `rendered/forms/` is the estate rendered from `plan-forms.yaml`, which keeps every service plain but ships the web stack's configs as a confext, announces services with DNS-SD, and gives the web stack a journal namespace, so the sysext, dnssd, and journald subtests have rendered input. `plugins/scripts/render-fixtures.sh` regenerates all three trees; the plan was drafted with `plan.ts` and `review.ts`, and its `generated_at` is pinned so the file is deterministic. When a component gains a decision, refresh each plan in place (`plan.ts inventory.json -o plan-machine.yaml` keeps the choices and adds the new decisions, then `review.ts plan-machine.yaml --accept-defaults`; the same for `plan-forms.yaml`), pin `generated_at` to `2026-09-01T12:00:00Z` again, and re-render.
