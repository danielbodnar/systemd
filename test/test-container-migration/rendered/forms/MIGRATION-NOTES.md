...
Rendered by systemd-migration from an inventory captured 2026-09-01T12:00:00Z (4 services, 2 nodes) against a plan of 52 decisions (52 chosen). Components composed, in order: machined, service, creds, resource-control, storage, networkd, resolved, journald, sysext, portable, quadlet.
...
- stack web: logs go to journal namespace web; read them with journalctl --namespace=web
- stack web: config files ship as confext web (decision sysext.configs.web); systemd-confext refresh merges them into /etc
